...
export default function ArsenalPanel() {
  ...
  const handleLaunchStrike = () => {
    ...
    if (targetId === countryId) {
      alert('Launch failed: Coordinates resolved to home country coordinates. Safety overrides active.');
      return;
    }
    ...
    if (currentWeapon.fuelLevel < 20) {
      alert(`Launch aborted: Insufficient weapon fuel reserves (${currentWeapon.fuelLevel}%). Refueling required.`);
      return;
    }
    ...
    useWorldStore.getState().addGlobalEvent(`WAR DEPARTMENT: Fired 1x ${selectedWeapon} targeting industrial zones of ${targetId}. Impact estimation: ${tickDist} ticks.`, 'CRITICAL');
  };
  ...
  const handleRefuelArsenal = () => {
    ...
    useWorldStore.getState().addGlobalEvent(`Logistics desk: Propellent supply re-injected into stockpiled flight engines. Total refuel executed.`, 'INFO');
  };

  return (
    <div className="w-full text-xs grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* Stockpile stats list */}
      <div className="flex flex-col gap-4">
                ...
                >
                  <div className="font-mono">
                    ...
                    <div className="text-[8px] text-gray-500 uppercase">
                      READY STATUS: {unit.operational} units | interception probability: {Math.round(unit.combatPowerRating)}%
                    </div>
                  </div>
                  ...
                  <div className="text-right font-mono flex items-center gap-3">
                    <div className="flex flex-col items-end">
                      ...
                      <span className={hasLowProp ? 'text-phosphor-red font-bold' : 'text-phosphor-green'}>
                        {unit.fuelLevel}%
                      </span>
                    </div>

                    <div className="flex flex-col items-end">
                      <span className="text-[8px] text-gray-500">QTY</span>
        ...
        <div className="combat-panel flex justify-around items-center h-20">
          ...
          <div className="text-center font-mono">
            <div className="text-[9px] text-gray-500 uppercase tracking-wider">Intercept Readiness</div>
            <div className="text-lg font-bold text-[#00e5ff]">ACTIVE</div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4">
        <div className="combat-panel flex flex-col gap-3">
          <h3 className="font-bold border-b border-[#1a3a1a] pb-1 uppercase tracking-wider text-phosphor-red">
          ...
          <div className="space-y-2">
            ...
            {/* Nuclear authorizations checkboxes */}
            {(selectedWeapon === 'ICBM' || selectedWeapon === 'SLBM') && (
              <div className="border border-red-950 p-2 rounded bg-red-950/10 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <label htmlFor="authNuke" className="text-phosphor-red font-bold text-[10px] cursor-pointer">
                    AUTHORIZE NUCLEAR SECOND-STRIKE PAYLOAD
                  </label>
                </div>

                {isNukeW && (
                  <div className="flex justify-between items-center text-[10px]">
                    <span className="text-red-500">WARHEAD EXPLOSIVE YIELD:</span>
                    <div className="flex gap-1.5 items-center">
                      <input
                        type="range"
                        min="0.2"
            ...
            <button
              ...
              className={`w-full py-2.5 font-bold uppercase rounded cursor-pointer transition-all ${
                !targetId
                  ...
                  : isNukeW
                  ? 'bg-[#40000a] border border-[#ff2244] text-[#ff2244] hover:bg-[#60000e] blink-hud shadow-lg shadow-[#ff2244]/15'
                  : 'bg-[#ffb300]/15 border border-[#ffb300] text-[#ffb300] hover:bg-[#ffb300]/30'
              }`}
            >
              {isNukeW ? '⚡ DEPLOY MASSIVE THERMONUCLEAR STRIKE' : '🚀 DISPATCH STRATEGIC ORDNANCE'}
            </button>
          </div>
        </div>

        {/* Real-time flight tracking telemetry list */}
        <div className="combat-panel flex-1 flex flex-col gap-1.5 min-h-[90px]">
          <span className="text-[9px] uppercase tracking-wider text-gray-500">FLIGHT TELEMETRY WARNING BOARD:</span>
          <div className="overflow-y-auto max-h-[80px] space-y-1 pr-1 font-mono text-[9px]">
            {activeStrikes.length === 0 ? (
              <p className="text-gray-600 italic">No ballistic tracks currently detected inside visual ranges.</p>
            ) : (
              activeStrikes.map((s) => {
